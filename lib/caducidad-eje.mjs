...
export async function calcularCaducidadEje() {
  ...
  for (let r = H + 1; r <= (ws.rowCount || H); r++) {
    ...
    const base = { ref: cuij || `exp ${expId}`, cuij, expId, caratula, fuero, impulso, verificado, meses, dias: Math.floor((hoy - impulso) / DIA_MS) };
    ...
    if (intimacion) {
      ...
      const it = { ...base, fase: "intimada", intimacion, venc, restan, nivel: restan < 0 ? "vencido" : (restan <= 10 ? "urgente" : "preventivo") };
    ...
    if (restan < 0) {
      ...
      // FASE EN CURSO. Se vuelca a la cartera siempre; al mail solo si esta en zona de aviso.
  ...
  return { items, todas, sinImpulso, excluidosAmparo, feriaCargada: feria.cargada, descuentaFeria: feria.descontar, nota: null };
}
