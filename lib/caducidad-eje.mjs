 ...
 *
 * Base normativa (aportada y confirmada por el abogado; texto consolidado Ley 6.764):
 *   Codigo Contencioso Administrativo y Tributario CABA, Ley 189 [VERIFICAR VIGENCIA]:
 *     - Art. 216: 6 meses de inactividad por causa imputable al ACTOR -> el tribunal
 *       intima; dentro de 30 dias el actor debe acreditar la prosecucion; si no, se
 *       declara la caducidad y se archiva. NO opera en procesos colectivos ni con
 *       interes publico/social/de tercero.
 *     - Art. 122: los plazos procesales se computan en dias habiles judiciales; la feria
 *       y los inhabiles se descuentan (feria-caba.json descontarEnCaducidad = true).
 *
...
// Suma n dias HABILES desde el dia siguiente (art. 22: el plazo corre desde el dia
// siguiente a la notificacion). Para la cuenta perentoria de 30 dias del art. 216.
...
export async function calcularCaducidadEje() {
  ...
  for (let r = H + 1; r <= (ws.rowCount || H); r++) {
    ...
    if (intimacion) {
      // FASE INTIMADA: cuenta perentoria de 30 dias habiles (art. 216 + 122).
...
export function renderCaducidadEje(res) {
  ...
  const lineaItem = (it) => {
    if (it.fase === "intimada") {
      ...
      return { txt: `[INTIMADA] ${it.ref} - ${it.caratula} | intimacion ${f(it.intimacion)} | 30 dias habiles vencen ${f(it.venc)} - ${r}`,
        html: `<li>[<b style="color:#8b1e1e">INTIMADA</b>] <b>${it.ref}</b> - ${it.caratula}<br><span style="color:#555">intimacion ${f(it.intimacion)} &middot; 30 dias habiles vencen <b>${f(it.venc)}</b> &middot; <b>${r}</b></span></li>` };
    }
  ...
  let texto = `>>> CADUCIDAD DE INSTANCIA CAyT - ${items.length} causa(s)${intimadas ? ` (${intimadas} INTIMADA/S)` : ""} (art. 216/122 Ley 189 [VERIFICAR VIGENCIA]) <<<\n`;
  let html = `<div style="border:2px solid #b58900;border-radius:4px;padding:8px 10px;margin:10px 0;background:#fdf6e3"><b style="color:#1e3a8b">CADUCIDAD DE INSTANCIA CAyT - ${items.length} causa(s)${intimadas ? ` &middot; ${intimadas} INTIMADA/S` : ""} (art. 216/122 Ley 189)</b><ul style="margin:6px 0">`;
  for (const it of items) { const l = lineaItem(it); texto += "  " + l.txt + "\n"; html += l.html; }
  texto += "\n";
  texto += `  Fases: EN CURSO (faltan dias para los ${CFG.meses} meses) -> HABILITADO PARA INTIMAR (6 meses cumplidos) -> INTIMADA (cargar "Fecha Notif. Intimacion": corre el perentorio de ${CFG.intimacionDias} dias habiles, art. 216/122). Solo cuenta la inactividad imputable al ACTOR.\n`;
  ...
  html += `</ul><div style="color:#555;font-size:12px"><b>Fases:</b> EN CURSO &rarr; HABILITADO PARA INTIMAR (6 meses) &rarr; INTIMADA (cargar "Fecha Notif. Intimacion" &rarr; perentorio de ${CFG.intimacionDias} dias habiles, art. 216/122 Ley 189 [VERIFICAR VIGENCIA]). Solo cuenta la inactividad imputable al ACTOR.<br><b>Excluidos por defecto:</b> amparos (Ley 2145 [VERIFICAR VIGENCIA]${res.excluidosAmparo ? `, ${res.excluidosAmparo} aqui` : ""}), colectivos e interes publico; forzar con "Caducidad Aplica" = SI.`;
