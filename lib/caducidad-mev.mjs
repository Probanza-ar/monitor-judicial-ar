 ...
 *
 * Reglas desde lib/reglas-plazos.mjs (JURIS.PBA). Articulado VERIFICADO texto literal
 * contra el consolidado oficial (normas.gba.gob.ar, jul-2026):
 *   - Art. 310 (t. Ley 13.986): 6 meses (1ra/unica instancia); 3 meses (2da/ulterior
 *     instancia y Justicia de Paz); 3 meses (sumario, sumarisimo, ejecutivo); o el plazo
 *     en que se opere la prescripcion de la accion, si fuere menor.
 *   - Art. 311 (t. Ley 12.357): corre desde la ultima peticion/resolucion/actuacion
 *     impulsoria, en dias inhabiles SALVO ferias judiciales (se descuentan) y descontando
 *     paralizacion/suspension. Computo conservador: meses corridos con feria descontada.
 *   - Art. 315 (t. Ley 13.986): NO se declara de plano. La pide POR UNICA VEZ el demandado
 *     (1ra inst.), el contrario del promotor (incidentes) o la recurrida (recursos). Se
 *     sustancia con INTIMACION PREVIA por unica vez: 5 dias para manifestar la intencion de
 *     continuar y producir actividad util. 2da fase: si la intimada activa y luego pasa
 *     igual plazo sin actividad util, se decreta a pedido o DE OFICIO. BIFASICA:
 *       "en curso"   -> no se cumplio el plazo del 310. Avisa al acercarse.
 *       "habilitado" -> plazo cumplido sin impulso: la CONTRAPARTE (en 1ra inst. la primera
 *                       peticion es de parte, no de oficio) puede pedir la caducidad; el
 *                       juzgado intima antes de declarar.
 ...
 * fuero Laboral (impulso de oficio; procedimiento laboral PBA Ley 15.057, vigente
 * desde feb-2020, deroga la vieja Ley 11.653; mod. Ley 15.557/2025). Familia y de
...
export async function calcularCaducidadMev() {
  ...
  const hoy = new Date();
  const items = [];
  const todas = []; // TODAS las causas con computo (en riesgo o no) -> para volcar a la cartera
  let sinImpulso = 0;
  ...
  for (let r = H + 1; r <= (ws.rowCount || H); r++) {
    ...
    const base = { ref: expediente || `nid ${nid}`, nidCausa: nid, pidJuzgado: juz, caratula, fuero: fuero || organismo, impulso, verificado, meses, dias: Math.floor((hoy - impulso) / DIA_MS) };
    ...
    if (intimacion) {
      ...
      const it = { ...base, fase: "intimada", intimacion, venc, restan, nivel: restan < 0 ? "vencido" : (restan <= 3 ? "urgente" : "preventivo") };
      todas.push(it); items.push(it);
      continue;
    }
    ...
    if (restan < 0) {
      const it = { ...base, fase: "habilitado", venc, restan, feriaDias, nivel: "habilitado" };
      todas.push(it); items.push(it);
    } else {
      const it = { ...base, fase: "encurso", venc, restan, feriaDias, nivel: restan <= 15 ? "urgente" : (restan <= CFG.avisoDias ? "preventivo" : "lejano") };
      todas.push(it);
      if (restan <= CFG.avisoDias) items.push(it);
    }
  }

  const ordenFase = { intimada: 0, habilitado: 1, encurso: 2 };
  items.sort((a, b) => (ordenFase[a.fase] - ordenFase[b.fase]) || (a.restan - b.restan));
  return { items, todas, sinImpulso, feriaCargada: feria.cargada, descuentaFeria: feria.descontar, nota: null };
}
...
export function renderCaducidadMev(res) {
  ...
  let texto = `>>> CADUCIDAD DE INSTANCIA PBA - ${items.length} causa(s)${intimadas ? ` (${intimadas} INTIMADA/S)` : ""} (arts. 310/311/315 CPCC BA, Dec-Ley 7.425/68 t. Ley 13.986/12.357) <<<\n`;
  ...
  texto += `  Excluidos automaticos: fuero Penal (rige prescripcion) y Laboral (impulso de oficio, Ley 15.057 proc. laboral PBA). Amparo NO se excluye por defecto [REVISION NORMATIVA REQUERIDA: criterio SCBA]. Overrides: "Caducidad Aplica" = SI/NO.\n`;
  ...
  html += `</ul><div style="color:#555;font-size:12px"><b>Plazos art. 310 (t. Ley 13.986):</b> 6 meses (1ra/unica) &middot; 3 meses (2da, paz, sumario, sumarisimo, ejecutivo) &middot; o la prescripcion si fuere menor (confirmar a mano). <b>Fases:</b> EN CURSO &rarr; PLAZO CUMPLIDO &rarr; INTIMADA (${CFG.intimacionDias} dias habiles perentorios, art. 315). En 1ra inst. la peticion es de PARTE (demandado), no de oficio.<br><b>Excluidos:</b> Penal y Laboral (impulso de oficio, Ley 15.057). Amparo computa por defecto [REVISION NORMATIVA REQUERIDA]. Overrides con "Caducidad Aplica".`;
