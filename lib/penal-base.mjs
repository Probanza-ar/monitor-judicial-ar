...
const RX_PARRAFO = [
  { rx: /ultimo\s*parr(?:afo)?\.?|in\s*fine/, val: "ult" },
  { rx: /(?:3\s*(?:er|ro|ero)?|tercer)\s*parr(?:afo)?\.?|\(\s*3\s*\)/, val: "3" },
  { rx: /(?:2\s*(?:do)?|segundo)\s*parr(?:afo)?\.?|\(\s*2\s*\)/, val: "2" },
  { rx: /(?:1\s*(?:er|ro|ero)?|primer)\s*parr(?:afo)?\.?|\(\s*1\s*\)/, val: "1" },
];
