...
export const TABLA_PENAS = {
  "79":  { max: 25, menor: false }, // homicidio simple (8 a 25 anios) -> termino topea en 12 (art.62.2)
...
// Diccionario NOMBRE del delito -> articulo del CP. Las caratulas penales traen el
// delito por nombre ("s/ HURTO", "s/ AMENAZAS"), no el numero. Solo se mapean delitos
// cuya pena esta en TABLA_PENAS (si no, buscarPena da null y se pide revision). Orden:
// del mas especifico al mas generico (las "lesiones gravisimas" antes que "lesiones").
export const TABLA_DELITOS = [
  { rx: /homicidio/, art: "79" },
  { rx: /lesiones\s+gravisimas/, art: "91" },
  { rx: /lesiones\s+graves/, art: "90" },
  { rx: /lesiones/, art: "89" },
  { rx: /abuso sexual|acceso carnal|violacion\b|ultraje sexual|estupro/, art: "119" },
  { rx: /corrupcion de menor|promocion.*prostitucion|facilitacion.*prostitucion/, art: "125" },
  { rx: /pornografia infantil|representacion.*menor/, art: "128" },
  { rx: /amenaza|coaccion/, art: "149bis" },
  { rx: /defraudacion|administracion fraudulenta|desbaratamiento/, art: "173" },
  { rx: /estafa/, art: "172" },
  { rx: /\brobo\b/, art: "164" },
  { rx: /\bhurto\b/, art: "162" },
];
...
export function articulosPorNombre(texto) {
  const t = norm(texto);
  const arts = [];
  for (const d of TABLA_DELITOS) if (d.rx.test(t) && !arts.includes(d.art)) arts.push(d.art);
  return arts.sort((a, b) => ((TABLA_PENAS[b]?.max ?? 0) - (TABLA_PENAS[a]?.max ?? 0)));
}
