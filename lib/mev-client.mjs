...
export async function listarDeptos() {
  ...
  if (!sel) return [];
  const out = [];
  for (const m of sel[1].matchAll(/<option[^>]*value=["']?([^"'>\s]*)["']?[^>]*>([\s\S]*?)<\/option>/gi)) {
    const nombre = sinTags(m[2]);
    if (nombre) out.push({ nombre, valor: m[1] });
  }
  return out;
}
...
function parseBusquedaForm(html) {
  ...
  if (selJuz) for (const m of selJuz[1].matchAll(/<option[^>]*value=["']([^"']*)["'][^>]*>([\s\S]*?)<\/option>/gi)) {
    const nombre = sinTags(m[2]);
    if (nombre) organismos.push({ nombre, valor: m[1] }); // OJO: valor con padding, no trimear
  }
  const sets = [];
  for (const selName of ["Set", "SetNovedades"]) {
    const sel = html.match(new RegExp(`<select[^>]*name=["']?${selName}["']?[^>]*>([\\s\\S]*?)<\\/select>`, "i"));
    if (!sel) continue;
    for (const m of sel[1].matchAll(/<option[^>]*value=["']?(\d+)["']?[^>]*>([\s\S]*?)<\/option>/gi)) {
      const nombre = sinTags(m[2]);
      if (nombre && !sets.some((s) => s.nidset === m[1])) sets.push({ nombre, nidset: m[1] });
    }
  }
