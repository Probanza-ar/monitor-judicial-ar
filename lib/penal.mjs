...
export async function calcularPenal() {
  ...
  for (let r = h + 1; r <= (wsC.rowCount || h); r++) {
    ...
    const tpDet = artDet ? buscarPena(artDet, parrDet) : null;
    if (!anios && artDet) {
      const t = tpDet ? terminoArt62(tpDet.max, tpDet.perpetua) : null;
      if (t) {
        anios = t;
    ...
    // Volcado a "Delito (art. CP)" y "Pena Max Anios": solo en celdas vacias, para no pisar
    // una carga manual (del abogado o de una corrida anterior).
    if (artDet) {
      const delitoVacio = cDelito && !String(row.getCell(cDelito).text || "").trim();
      const penaVacia = cPena && !String(row.getCell(cPena).text || "").trim();
      const etiqueta = parrDet ? ` (${ETIQUETA_PARRAFO[parrDet] || parrDet})` : "";
      const penaAuto = (penaVacia && tpDet && Number.isFinite(tpDet.max)) ? tpDet.max : null;
      if (delitoVacio || penaAuto != null) {
        detecciones.push({ fila: r, valor: delitoVacio ? `${artDet}${etiqueta}` : null, pena: penaAuto });
      }
    }
  ...
  if (detecciones.length && (cDelito || cPena)) {
    try {
      ...
      for (const d of detecciones) {
        if (d.valor != null && cDelito) wsC.getRow(d.fila).getCell(cDelito).value = d.valor;
        if (d.pena != null && cPena) wsC.getRow(d.fila).getCell(cPena).value = d.pena;
      }
