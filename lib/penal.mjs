...
import { terminoArt62, articuloDe, TABLA_PENAS } from "./penal-base.mjs";
...
export async function calcularPenal() {
  ...
  const hoy = new Date();
  const inactividad = [], prescripcion = [];
  for (let r = h + 1; r <= (wsC.rowCount || h); r++) {
    ...
    if (!anios) { // prefill por articulo del CP en la caratula (tabla compartida penal-base)
      const tp = TABLA_PENAS[articuloDe("", caratula)];
      const t = tp ? terminoArt62(tp.max) : null;
      if (t) { anios = t; fteAnios = "de tabla (art. CP) [VERIFICAR VIGENCIA]"; }
    }
    ...
    if (anios && base) {
      ...
      if (restan <= avisoDias) {
        const nivel = restan < 0 ? "prescripta" : (restan <= 30 ? "urgente" : "preventivo");
        prescripcion.push({ nro, caratula, fuero, base, desde: interr ? "ult. acto interruptivo" : "fecha del hecho", anios, fteAnios, prescribe, restan, nivel });
      }
    }
  }
  ...
  return { inactividad, prescripcion, nota: null };
}

export function renderPenal({ inactividad, prescripcion }) {
  if ((!inactividad || !inactividad.length) && (!prescripcion || !prescripcion.length)) return null;
