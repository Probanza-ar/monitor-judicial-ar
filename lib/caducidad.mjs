...
import { JURIS, plazoCaducidadMeses } from "./reglas-plazos.mjs";
...
function fueroExcluido(fuero, caratula) {
  ...
  if (/penal|criminal|correccional|casacion penal|s\/ ?inf/.test(t)) return "penal";
  if (/laboral|trabajo|\bcnt\b/.test(t)) return "laboral";
  return null;
}
...
function plazoEstimado(fuero, estado, obs) {
  return plazoCaducidadMeses(JURIS.NACION, { texto: `${fuero} ${estado} ${obs}` });
}
...
export async function calcularCaducidad() {
  ...
  for (let r = h + 1; r <= (wsC.rowCount || h); r++) {
    ...
    if (!meses) { meses = plazoEstimado(fuero, estado, obs) || mesesDefault; ftePlazo = "estimado"; }
    ...
    items.push({ nro, caratula, fuero, impulso, dias, meses, ftePlazo, impulsoVerificado, feriaDias, susDias, venc: vencHabil, prorrogado, restan, nivel });
  }

  items.sort((a, b) => a.restan - b.restan);
  return { items, sinImpulso, nota: null, ferias: ferias.length };
}

export function renderCaducidad(items, sinImpulso = 0) {
  ...
  for (const it of items) {
    ...
    // Escalada: sin acto impulsorio verificado = revision requerida, no simple aviso.
    const etiqueta = it.impulsoVerificado ? et[it.nivel] : "REVISION REQUERIDA";
    const nota = it.impulsoVerificado ? "" : " -- causa sin acto impulsorio verificado (el ultimo movimiento puede no interrumpir la caducidad)";
    ...
    const color = it.impulsoVerificado ? "#555" : "#8b1e1e";
    html += `<li>[<b>${etiqueta}</b>] <b>${it.nro}</b> - ${it.caratula}<br><span style="color:${color}">impulso ${f(it.impulso)}${it.impulsoVerificado ? "" : " <i>(estimado)</i>"} &middot; plazo ${it.meses} meses (${it.ftePlazo}) &middot; vence <b>${f(it.venc)}</b>${grac}${desc} &middot; ${restanTxt}${it.impulsoVerificado ? "" : "<br><b>Sin acto impulsorio verificado: el ultimo movimiento puede no interrumpir la caducidad.</b>"}</span></li>`;
  }
  texto += "\n";
  if (sinImpulso > 0) {
