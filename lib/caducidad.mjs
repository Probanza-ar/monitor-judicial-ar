...
export async function calcularCaducidad() {
  ...
  for (let r = h + 1; r <= (wsC.rowCount || h); r++) {
    ...
    const nivel = restan < 0 ? "vencido" : (restan <= 15 ? "urgente" : (restan <= avisoDias ? "preventivo" : "lejano"));
    const it = { nro, caratula, fuero, impulso, dias, meses, ftePlazo, impulsoVerificado, feriaDias, susDias, venc: vencHabil, prorrogado, restan, nivel, tipoProceso };
    todas.push(it); // se vuelca a la cartera aunque no este en riesgo
    if (restan > avisoDias) continue; // el mail solo lista las que estan en zona de aviso
    if (tipoProceso) sinAplicabilidad++;
    items.push(it);
  }

  items.sort((a, b) => a.restan - b.restan);
  return { items, todas, sinImpulso, sinAplicabilidad, nota: null, ferias: ferias.length };
}
